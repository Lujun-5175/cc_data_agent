...
_VISION_PROVIDERS = frozenset({
    ...
    "kimi",        # Moonshot (vision-capable)
})


def _supports_vision(config: dict) -> bool:
    """Check if the current model/provider supports image inputs."""
    model = config.get("model", "")
    provider = detect_provider(model)
    if provider in _VISION_PROVIDERS:
        return True
    # Ollama with vision-specific models
    if provider == "ollama":
        vision_models = {"llava", "bakllava", "cogvlm", "moondream", "minicpm-v"}
        model_lower = model.lower().replace("ollama/", "")
        return any(vm in model_lower for vm in vision_models)
    # LM Studio depends on loaded model — assume no vision
    return False


def _save_image_for_ocr(b64_data: str, config: dict, user_msg: dict) -> None:
    """Save base64 image to temp file and append OCR-friendly user message."""
    ...
    try:
        ...
        # If decoding fails, just let the message through without image
        return

    # Use TEMP dir or fall back to CWD
    tmp = Path(os.environ.get("TEMP", "."))
    fname = f"agent_img_{uuid.uuid4().hex[:8]}.png"
    img_path = tmp / fname
    img_path.write_bytes(raw)

    ocr_hint = (
        f"\n\n[Image saved to `{img_path}` — this model cannot "
