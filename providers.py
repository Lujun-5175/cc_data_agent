...
def messages_to_anthropic(messages: list) -> list:
    ...
    Images in user messages (``m["images"]``) are mapped to Anthropic's
    ``image`` content-block format so vision works with Claude models.
    """
    result = []
    i = 0
    while i < len(messages):
        ...
        if role == "user":
            ...
            images = m.get("images")
            if images:
                # Anthropic uses content-block format with inline image sources
                blocks = [{"type": "text", "text": content}]
                for b64 in images:
                    # b64 is raw base64 (providers.py adds media_type prefix)
                    blocks.append({
                        "type": "image",
                        "source": {
                            "type": "base64",
