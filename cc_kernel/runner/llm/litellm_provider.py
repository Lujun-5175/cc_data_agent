...
class LiteLLMProvider:
    ...
    def _build_messages(self, request: LlmRequest) -> list[dict]:
        ...
        Handles the ``images`` field on user messages by converting to
        OpenAI/Gemini multipart ``image_url`` format so vision-capable
        models receive the image data correctly.
        """
        if request.messages:
            messages = []
