...
class ChatSession:
    ...
    def _busy_error_payload(self) -> dict:
        return {"type": "error", "data": {"message": "Agent is busy"}}

    def _try_begin_turn(self) -> bool:
    ...
    def _start_turn_thread(self, target) -> None:
    ...
    def _broadcast(self, event: ChatEvent):
        with self._sub_lock:
            ...
            # Push to live subscribers; prune dead (full) ones
    ...
    def handle_slash_sync(self, line: str) -> list[dict]:
        ...
        if not self._try_begin_turn():
            return [self._busy_error_payload()]
    ...
    def handle_slash_stream(self, line: str, event_callback):
        ...
        """
        if not self._try_begin_turn():
            event_callback(self._busy_error_payload())
            return
    ...
    def submit_prompt(
        ...
        if prompt.startswith("/"):
            handled = None
            try:
                handled = self._handle_slash(prompt)
                return True
            finally:
                if handled != _ASYNC_HANDLED:
                    ctx.pending_image = None
        ...
        try:
            ...
            def _run():
                try:
                    ...
                    self._end_turn()
                    # Clear event buffer — turn is done, no need to replay
                    # stale events when a late-joining WS subscriber connects.
                    with self._sub_lock:
                        self._event_buffer.clear()
                    self._broadcast(ChatEvent("status", {"state": "idle"}))

            self._start_turn_thread(_run)
            return True
        except Exception:
            self._end_turn()
            raise
    ...
    def _handle_slash(self, line: str) -> bool:
        ...
        if cmd_name == "ssj" and cmd_args.split()[0].lower() in _SSJ_DIRECT:
            ...
            if sub == "todo":
                ...
                self._start_turn_thread(_run_ssj)
                return _ASYNC_HANDLED
        ...
        if cmd_name in _LONG_RUNNING:
            ...
            self._start_turn_thread(_run_long)
            return _ASYNC_HANDLED
        ...
        if isinstance(result, tuple):
            ...
            self._start_turn_thread(_run_sentinel)
            return _ASYNC_HANDLED
...
def _evict_chat_session_cache(sid: str) -> bool:
    """Drop a live ChatSession from memory without touching persisted data."""
    with _chat_lock:
        session = _chat_sessions.pop(sid, None)
    if session:
        session.cleanup()
...
def reap_stale_chat_sessions():
    """Evict stale in-memory sessions while preserving the DB-backed history.

    The web UI intentionally persists sessions in SQLite so they survive
    refreshes and restarts. The reaper should only free idle runtime state,
    not delete the user's conversation history.
    """
    stale: list[str] = []
    with _chat_lock:
