...
def _handle_chat_websocket(sock: socket.socket, extra: bytes,
    ...
    def _ws_reader():
        try:
            ...
            get_logger("ws_reader").exception("unexpected error in WS reader")
            reader_alive.clear()
    ...
    try:
        ...
        if path == "/api/auth/whoami" and method == "GET":
            ...
            try:
                ...
                user = None
            if not user and _server_no_auth:
                # No-auth mode: create a synthetic user on the fly so the
                # chat UI doesn't get stuck in a 401 loop when the users
                # table is empty but sessions already exist (e.g. after a
                # DB reset or schema migration).
                try:
                    from web.auth import hash_password
                    user = dbrepo.create_user(
                        "default", hash_password("default"),
                        is_admin=True,
                    )
                except Exception:  # noqa: BLE001
