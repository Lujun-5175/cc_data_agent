...
from .store import MemoryEntry, load_index
...
def _memory_save(params: dict, config: dict) -> str:
    ...
    import sys
    # Force re-import of store.py so file-encoding fix takes effect
    # without requiring a REPL restart.
    sys.modules.pop('memory.store', None)
    from .store import save_memory, check_conflict, MemoryEntry
...
def _memory_delete(params: dict, config: dict) -> str:
    ...
    from .store import delete_memory  # lazy import
    name = params["name"]
    scope = params.get("scope", "user")
...
def _memory_search(params: dict, config: dict) -> str:
    ...
    from .store import touch_last_used  # lazy import
    for r in results:
        if r.get("file_path"):
