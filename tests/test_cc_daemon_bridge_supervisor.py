"""Tests for cc_daemon/bridge_supervisor.py (RFC 0002 F-6/7/8 skeleton).
...
class TestLifecycle(_BridgeTestBase):
    ...
    def test_start_rejects_while_previous_stop_is_still_joining(self):
        from cc_daemon import bridge_supervisor as bs
        os.environ["CHEETAHCLAWS_ENABLE_F6"] = "1"
        ...
        live_thread.start()
        started = bs.BridgeHandle(
            kind="telegram",
            config={"telegram_token": "fake", "telegram_chat_id": 1},
        ...
        with bs._handles_lock:
            bs._handles["telegram"] = started
        try:
            with self.assertRaises(RuntimeError):
                bs.start("telegram", {"telegram_token": "fake", "telegram_chat_id": 1})
        finally:
            release.set()
            started.thread.join(timeout=1.0)
            with bs._handles_lock:
                current = bs._handles.get("telegram")
                if current is started:
                    bs._handles.pop("telegram", None)
...
class TestSqlitePersistence(_BridgeTestBase):
    ...
    def test_db_failure_does_not_raise(self):
        ...
        with patch("cc_daemon.schema.get_conn",
